...
-- Modifies _G.CONFIG_CONTENTS global variable
M.load_config = function(reload)
   -- only do the stuff below one time, otherwise just return the set config
   if _G.CONFIG_CONTENTS ~= nil and not (reload or false) then
      return _G.CONFIG_CONTENTS
   end
   ...
   local config_name = vim.g.user_config
   ...
   _G.CONFIG_CONTENTS = require(config_name)
   ...
   if vim.fn.filereadable(vim.fn.glob(config_file)) == 1 then
      ...
      if present then
         ...
         if type(config) == "table" then
            ...
            _G.CONFIG_CONTENTS = require("core.utils").merge_table(
               _G.CONFIG_CONTENTS,
               config,
               to_replace
            )
